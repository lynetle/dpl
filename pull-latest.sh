...
if [ "$NEEDS_UPDATE" = true ]; then
    ...
    # --- [OPTIMIZATION START] ---
    # 步骤 0: 在任何操作前，获取当前本地镜像的ID，以便后续精确删除
    echo "0/5: 正在记录当前镜像ID..."
    OLD_IMAGE_ID=$(docker image inspect --format='{{.Id}}' "$IMAGE_VALUE" 2>/dev/null || true)
    if [ -n "$OLD_IMAGE_ID" ]; then
        echo "    -> 旧镜像ID为: $OLD_IMAGE_ID"
    else
        echo "    -> 本地不存在当前镜像，无需记录。"
    fi
    # --- [OPTIMIZATION END] ---

    echo "1/5: 正在更新 $COMPOSE_FILE..."
    ...
    echo "2/5: 正在拉取新镜像: ${IMAGE_REPO}:${NEW_TAG}..."
    ...
    # --- [OPTIMIZATION START] ---
    # 步骤 4: 精确清理被替换下的旧镜像
    echo "4/5: 正在清理被替换下的旧镜像..."
    if [ -n "$OLD_IMAGE_ID" ]; then
        # 再次确认新镜像的ID，防止误删
        NEW_IMAGE_ID=$(docker image inspect --format='{{.Id}}' "${IMAGE_REPO}:${NEW_TAG}" 2>/dev/null)
        if [ "$OLD_IMAGE_ID" != "$NEW_IMAGE_ID" ]; then
            echo "    -> 准备删除旧镜像 (ID: $OLD_IMAGE_ID)..."
            # 使用 -f 强制删除，以处理旧镜像可能存在的多个标签（例如更新前后的`latest`标签）
            if docker rmi -f "$OLD_IMAGE_ID" >/dev/null 2>&1; then
                echo "    ✅ 旧镜像已成功删除。"
            else
                # 这种情况很少见，但可能发生，例如该旧镜像仍被另一个手动启动的容器使用
                echo "    ⚠️ 未能删除旧镜像 (ID: $OLD_IMAGE_ID)。可能仍被其他容器或标签引用。"
            fi
        else
            echo "    -> 新旧镜像ID相同（内容未变），无需清理。"
        fi
    else
        echo "    -> 本地不存在旧镜像，无需清理。"
    fi
    ...
    echo "5/5: 清理可能产生的悬空镜像 (dangling images)..."
    # 在精确删除后，可以额外运行一次这个来清理因拉取失败等原因产生的<none>:<none>镜像
